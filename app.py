...
# 1. KONFIGURASI
st.set_page_config(page_title="StockPro Precision v3.5", layout="wide")
...
if not df_w.empty:
    ...
    # KOMPONEN SHOW/HIDE LIST
    with st.expander("📋 Lihat Daftar Pantau (Watchlist)", expanded=True):
        if not df_s.empty:
            # Menampilkan dalam bentuk tabel bersih tanpa index
            st.dataframe(
...
with ca:
    target = st.text_input("🔍 Masukkan Kode Saham (Manual):", value="BBRI").upper()
with cb:
...
try:
    ...
    st.error(f"Emiten tidak ditemukan atau data bermasalah. (Detail: {e})")
