...
txt_input = st.sidebar.text_area("Input Kode (Tanpa .JK, pisah koma):", 
                                "BBRI, TLKM, ASII, ADRO, GOTO, BMRI, BBNI, UNTR, AMRT, BRIS, BBCA, ANTM, MDKA, PTBA, ITMG, CPIN, UNVR, ICBP, INDF, PGAS")
# Membersihkan input dan menambah .JK
...
def get_data_watchlist(tickers):
    ...
    for t in tickers:
        try:
            # Mengambil data 2 hari terakhir untuk hitung perubahan harga
...
waktu_live = datetime.now(tz).strftime('%d %b %Y | %H:%M:%S')
...
if not df_w.empty:
    # Filter harga berdasarkan input manual sidebar
    df_f = df_w[(df_w['Harga'] >= min_h) & (df_w['Harga'] <= max_h)]
    df_s = df_f.sort_values(by="Chg%", ascending=False).reset_index(drop=True)
    
    st.subheader(f"🏆 Performa Watchlist (Limit 30 Emiten)")
    
    if not df_s.empty:
        # Menampilkan 5 metrik teratas
        ...
        # Tabel lengkap untuk semua emiten yang diinput
        with st.expander("📂 Klik untuk Lihat Seluruh Daftar Watchlist"):
            st.dataframe(df_s, use_container_width=True, hide_index=True)
    else:
        st.warning(f"Tidak ada emiten dalam rentang harga Rp {min_h:,} - Rp {max_h:,}. Sesuaikan filter di sidebar.")
else:
    st.error("Gagal menarik data. Pastikan kode emiten benar dan internet stabil.")
...
with ca:
    # Input manual untuk saham yang ingin dianalisa detail
    target_input = st.text_input("🔍 Ketik Kode Saham Analisis (Contoh: BBCA):", 
                                value=df_s.iloc[0]['Ticker'] if not df_s.empty else "BBRI").upper()
with cb:
...
try:
    ...
    if not df.empty:
        ...
        # --- HARGA REAL ANALISIS ---
        st.markdown(f"### 📈 Live Data: {target_input}.JK")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Harga Terkini", f"Rp {cl[-1]:,.0f}", f"{cl[-1]-op[-1]:,.0f}")
        m2.metric("RSI (14)", f"{rsi.iloc[-1]:.2f}")
        m3.metric("Volume", f"{vl[-1]:,.0f}")
        m4.metric("Prediksi 10P", f"Rp {future_prices[-1]:,.0f}")

        # --- GRAFIK MULTI-PANEL ZOOMABLE ---
        fig = make_subplots(rows=4, cols=1, shared_xaxes=True, 
                           vertical_spacing=0.03, row_heights=[0.4, 0.15, 0.2, 0.25],
                           subplot_titles=("Price & BB & Prediction", "Volume", "MACD", "RSI"))

        # Panel 1: Candle & BB & Prediksi
        ...
        fig.update_layout(
            ...
            xaxis_rangeslider_visible=True, # Slider fleksibel
    ...
    st.error(f"Gagal memuat grafik {target_input}. Pastikan kode benar.")
